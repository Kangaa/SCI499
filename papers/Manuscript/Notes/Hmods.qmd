...
GMelb_Popns[[4]] <- tibble("GCC_Code" = "2GMEL",
...
HPmixmat <- function(data){
  ...
  for (i in 1:npatch){
    ...
    for (j in 1:npatch){
      patch_j <- data[j,]
      for (l in seq_along(levels)){ #find lowest level of association (LLA)
        if (patch_i[[levels[l]]] == patch_j[[levels[l]]]){
          #mixmat[i,j] <- mixmat[i,j] + (xi[l]/norm[l])
          if (l == 1) mixmat[i,j] <- xi[l]


          else {
            ll_pop <- GMelb_Popns[[1]][GMelb_Popns[[1]][[glue("{substr(levels[1], 1,3)}_Code")]] == patch_j[[levels[1]]],][[3]]
            ul_pop <- GMelb_Popns[[l]][GMelb_Popns[[l]][[glue("{substr(levels[l], 1,3)}_Code")]] == patch_i[[levels[l]]],][[3]] - GMelb_Popns[[l-1]][GMelb_Popns[[l-1]][[glue("{substr(levels[l-1], 1,3)}_Code")]] == patch_i[[levels[l-1]]],][[3]]
