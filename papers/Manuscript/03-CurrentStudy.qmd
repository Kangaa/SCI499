# Metapopulation Spatial resolution {#sec-intro}
...
### Mixing matrices to preserve homogeneous mixing
...
$$
Where $P_{tot}$ is the total number of patches. 
- How should we interpret this mixing coefficient?

The mixing matrix $\phi$ entails that mixing is uniform across all patches, and that the mixing coefficient is independent of the size of the patch. This is equivalent to assuming that the population of each patch is the same, and that the mixing coefficient is independent of the size of the patch. However, In the case of the GMGCCSA this is not the case, as the population of each SA2 region varies considerably @fig-GMelbSA2Popn.

### Population proportional mixing
To account for the different population sizes of each patch, we can construct a mixing matrix where the mixing coefficient is proportional to the population size of each patch i.e.. for two patches $i$ and $j$ the mixing coefficient $\phi_{i,j}$:
...
-Duration
d
of pop prop models with SA2 SA3 SA4 homogeneous patches - Expect equivalence
...
$$ thus for the row $M_{i}$, representing the mixing of individuals from Kensington, \$ sum\limits\*{j}M\*{j}= \frac{1}{4} +\frac{1}{8} + \frac{1}{8} +\frac{1}{8} + \frac{1}{8} + \frac{1}{8} + \frac{1}{8} = 1\$ as desired. Repeating this for all patches $i$, gives the mixing matrix represented in @fig-egMM
