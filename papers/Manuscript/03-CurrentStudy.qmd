...
The intra level mixing coefficients $\xi_L$ comprise the vector $\xi$. Constraining the elements of $\xi$ s.t. $\sum\limits_{i = 1}^{L} \xi_i = 1$ (where $L$ is the highest level grouping) ensures the rows of the mixing matrix sum to unity. 
